...
csv_path = os.path.join(os.path.dirname(__file__), "..", "..", "..", "ML", "Dataset", "Diabetes_Final_Data_V2.csv")
RANDOM_STATE = 42
...
def train_and_evaluate_xgboost(X_train, X_test, y_train, y_test):
    ...
    # Model saving logic moved to main execution block.
    return xgb_clf
...
if __name__ == "__main__":
    ...
    xgb_clf = train_and_evaluate_xgboost(X_train_resampled, X_test_scaled, y_train_resampled, y_test)

    # Save the model and scaler
    import joblib
    joblib.dump(xgb_clf, os.path.join(os.path.dirname(__file__), 'xgboost_model.pkl'))
    joblib.dump(scaler, os.path.join(os.path.dirname(__file__), 'scaler.pkl'))
    print("\nModel and scaler successfully saved to xgboost_model.pkl and scaler.pkl")
